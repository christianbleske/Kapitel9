...
class ViewController: UIViewController {
    ...
    @IBAction func uiButtonEmbeddedHtml_Pressed(_ sender: AnyObject) {
        embeddedHTML()
    }

    @IBAction func uiButtonHtmlFromFile_Pressed(_ sender: AnyObject) {
        loadHTMLFromFile()
    }

    @IBAction func uiButtonHtmlFromFileWithImage_Pressed(_ sender: AnyObject) {
        loadHTMLFromFileWithImage()
    }
    
    func loadHTMLFromFile() {
        let url = URL(fileURLWithPath: Bundle.main.path(forResource: "htmlpage", ofType: "htm")!)
    ...
    func loadHTMLFromFileWithImage() {
        let url = URL(fileURLWithPath: Bundle.main.path(forResource: "htmlpage2", ofType: "htm")!)
